...
export default class ScrollableTabView extends Component {
  ...
  componentDidMount () {
    let scene = this._children()[this.state.currentPage]
    if (scene.props.onFocus) {
      scene.props.onFocus()
    }
  }
  ...
  _onChangeTab = (prevPage, currentPage) => {
    ...
    let prevScene = this._children()[prevPage]
    if (prevScene.props.onBlur) {
      prevScene.props.onBlur()
    }
    setTimeout(() => {
      let scene = this._children()[currentPage]
      if (scene.props.onFocus) {
        scene.props.onFocus()
      }
    }, 300)
