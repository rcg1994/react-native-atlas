...
var ModalBox = createReactClass({
  ...
  animateClose: function () {
    ...
    this.state.animClose = Animated.timing(
      this.state.position,
      {
        toValue: this.props.entry === 'top' ? -this.state.containerHeight : this.state.containerHeight + 300,
        duration: this.props.animationDuration
      }
    )
  ...
  renderContent () {
    ...
    return (
      <Animated.View
        ...
        style={[styles.wrapper, size, this.props.style, this.props.float ? null : {transform: [{translateY: this.state.position}, {translateX: offsetX}]}]}
  ...
  render: function () {
    ...
    var content = (
      <View style={[styles.transparent, styles.absolute]} pointerEvents={'box-none'}>
        <View style={{flex: 1, position: 'relative'}} pointerEvents={'box-none'} onLayout={this.onContainerLayout}>
    ...
    return (
      <Modal onRequestClose={() => this.close()} supportedOrientations={['landscape', 'portrait']} transparent
             visible={visible}>
        {content}
      </Modal>
    )
