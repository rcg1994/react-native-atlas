...
class Header extends Component {
  ...
  componentWillMount () {
    ...
    //   Helper.eventManage.emit('status/set', statusBar)
    // })
  }
  ...
  render () {
    ...
    let headerPaddingTop = fixStatusBar ? (fixPaddingTop || Theme.header_padding_top) : 0
    ...
    let headerLeftNode = headerLeft || (
      ...
      >
        {
          isDark ? <Image style={styles.arrowImage} source={require('../assets/back.png')} resizeMode='contain' />
            : <Image style={styles.arrowImage} source={require('../assets/back_light.png')} resizeMode='contain' />
        }
      </IconButton>
    )
    if (headerLeft === null) {
    ...
    return (
      <View style={[styles.header, {backgroundColor: isDark ? '#fff' : Colors.primary}, commonStyle, style]}>
        <View style={[styles.headerLeft, commonStyle, headerLeftStyle]}>
          {headerLeftNode}
        </View>
        <Text style={[styles.title, {
          color: isDark ? Theme.header_title_font_dark_color : Theme.header_title_font_light_color
